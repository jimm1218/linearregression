...
with col_chart:
    ...
    ax.scatter(x, y, alpha=0.6, label='Data Points', color='skyblue')
    ax.plot(x, a_true * x + b_true, 'g--', linewidth=2, label=f'True Line (a={a_true:.2f})')
    ax.plot(x, y_pred, 'r-', linewidth=2, label=f'Regression Line (a={a_fit:.2f})')
    
    # 標示出離群值
    ax.scatter(
        top_10_outliers['x'], top_10_outliers['y_true'], 
        s=150, facecolors='none', edgecolors='red', linewidths=2, label='Top 10 Outliers'
    )
